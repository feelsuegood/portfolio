...
function parsePortfolioSections(content: string) {
  ...
  let currentSkillCategory: keyof PortfolioData["skills"] | "" = "";
  let currentProject: PortfolioData["projects"][number] | null = null;
  let i = 0;
  ...
  while (i < lines.length) {
    ...
    if (line === "**Programming Languages**") {
      ...
      (result.skills[currentSkillCategory] as string[]).push(
        line.replace("- ", ""),
      );
    }
